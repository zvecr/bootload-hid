...
class BootloadHID extends EventEmitter {
  ...
  async getBoard() {
    const info = HID.devices().find(isBootloadHID);
    if (!info) {
  ...
  async programBoard(device, hex, pageSize, deviceSize, startAddr, endAddr) {
    ...
    this.emit('flash:start', null, startAddr, endAddr, pageSize, deviceSize);
