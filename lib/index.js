...
class BootloadHID extends EventEmitter {
  ...
  getBoardInfo(device) {
    const buf = Buffer.from(device.getFeatureReport(1, 8));

    return {
      pageSize: buf.readUIntLE(2, 2),
      deviceSize: buf.readUIntLE(4, 4),
    };
  }
  ...
  static list(callback) {
    let deviceInfo;
    try {
      deviceInfo = HID.devices().filter(isBootloadHID);
    } catch (e) {
      callback(e);
    }

    // call outside try/catch to propagate callback throws
